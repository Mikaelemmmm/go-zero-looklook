...
var (
	...
	cacheSysOaHomestayOrderIdPrefix = "cache:sysOa:homestayOrder:id:"
	cacheSysOaHomestayOrderSnPrefix = "cache:sysOa:homestayOrder:sn:"
)
...
func (m *defaultHomestayOrderModel) Insert(ctx context.Context, session sqlx.Session, data *HomestayOrder) (sql.Result, error) {
	...
	sysOaHomestayOrderIdKey := fmt.Sprintf("%s%v", cacheSysOaHomestayOrderIdPrefix, data.Id)
	sysOaHomestayOrderSnKey := fmt.Sprintf("%s%v", cacheSysOaHomestayOrderSnPrefix, data.Sn)
	...
	}, sysOaHomestayOrderIdKey, sysOaHomestayOrderSnKey)
}

func (m *defaultHomestayOrderModel) FindOne(ctx context.Context, id int64) (*HomestayOrder, error) {
	sysOaHomestayOrderIdKey := fmt.Sprintf("%s%v", cacheSysOaHomestayOrderIdPrefix, id)
	var resp HomestayOrder
	err := m.QueryRowCtx(ctx, &resp, sysOaHomestayOrderIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultHomestayOrderModel) FindOneBySn(ctx context.Context, sn string) (*HomestayOrder, error) {
	sysOaHomestayOrderSnKey := fmt.Sprintf("%s%v", cacheSysOaHomestayOrderSnPrefix, sn)
	var resp HomestayOrder
	err := m.QueryRowIndexCtx(ctx, &resp, sysOaHomestayOrderSnKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) (i interface{}, e error) {
...
func (m *defaultHomestayOrderModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheSysOaHomestayOrderIdPrefix, primary)
}
