...
func (l *UpdateHomestayOrderTradeStateLogic) verifyOrderTradeState(newTradeState, oldTradeState int64) error {
	...
	if newTradeState == model.HomestayOrderTradeStateWaitPay {
		return errors.Wrapf(xerr.NewErrMsg("不支持更改此状态"), "不支持更改为待支付状态 newTradeState : , oldTradeState:%d", newTradeState, oldTradeState)
	}

	if newTradeState == model.HomestayOrderTradeStateCancel {

		if oldTradeState != model.HomestayOrderTradeStateWaitPay {
			return errors.Wrapf(xerr.NewErrMsg("只有待支付的订单才能被取消"), "只有待支付的订单才能被取消 newTradeState : , oldTradeState:%d", newTradeState, oldTradeState)
		}

	} else if newTradeState == model.HomestayOrderTradeStateWaitUse {
		if oldTradeState != model.HomestayOrderTradeStateWaitPay {
			return errors.Wrapf(xerr.NewErrMsg("只有待支付的订单才能更改为此状态"), "只有待支付的订单才能更改为未使用状态 newTradeState : , oldTradeState:%d", newTradeState, oldTradeState)
		}
	} else if newTradeState == model.HomestayOrderTradeStateUsed {
		if oldTradeState != model.HomestayOrderTradeStateWaitUse {
			return errors.Wrapf(xerr.NewErrMsg("只有未使用的订单才能更改为此状态"), "只有未使用的订单才能更改为已使用状态 newTradeState : , oldTradeState:%d", newTradeState, oldTradeState)
		}
	} else if newTradeState == model.HomestayOrderTradeStateRefund {
		if oldTradeState != model.HomestayOrderTradeStateWaitUse {
			return errors.Wrapf(xerr.NewErrMsg("只有未使用的订单才能更改为此状态"), "只有未使用的订单才能更改为退款状态 newTradeState : , oldTradeState:%d", newTradeState, oldTradeState)
		}
	} else if newTradeState == model.HomestayOrderTradeStateExpire {
		if oldTradeState != model.HomestayOrderTradeStateWaitUse {
			return errors.Wrapf(xerr.NewErrMsg("只有未使用的订单才能更改为此状态"), "只有未使用的订单才能更改为已过期状态 newTradeState : , oldTradeState:%d", newTradeState, oldTradeState)
		}
	}
